          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div class="flex justify-center gap-4">
              <button
                ...
                @click="joinQueue"
                :disabled="loading"
                class="px-6 py-3 bg-[#FFF9C4] text-zinc-600 rounded-lg font-medium hover:bg-[#FFF176] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                <span
                  v-if="loading"
                  class="animate-spin h-4 w-4 border-2 border-zinc-600 border-t-transparent rounded-full"
                ></span>
                {{ loading ? "Chamando..." : "Próximo da fila" }}
              </button>
            </div>
          </div>
        </div>
